...
public class Cache_FuncTmp extends Cache{
    ...
    public void Cache_Access(Block_Func Input_block)
    {
        ...
        {
           if(Block_recency.contains(Input_block)) //Always HIT
           {
               count++;
           ...
           else //first acc
           {
               ...
               else if( Block_residentHIR.size() == Size_HIR-1)
               {
                ...
                switch(Input_block.getBlock_status()) //stk_reaccess
           {
               ...
               case Block_Value.RESIDENT_HIR: 
                   if(Stk_isReaccess(Input_block)) 
                   {
                       ...
                       IRR_calculator(Input_block);
                       Stk_MvTop(Input_block);
                       Qarray = (Block_Func [])Block_LIR.toArray();
                       if(Qarray[0].getBlock_irr() >= Input_block.getBlock_irr())
                       {
    ...
    {
      Iterator<LIRS_RDFILE_OBJECT.Block_Func> Iterator_recency = Block_recency.iterator(); //start at first
      LIRS_RDFILE_OBJECT.Block_Func Tmp_block;
    ...
    public void IRR_calculator(LIRS_RDFILE_OBJECT.Block_Func Input_block)
    {
            boolean Is_inf = true;
    ...
    public void Stk_MvTop(LIRS_RDFILE_OBJECT.Block_Func Input_block) 
    {
