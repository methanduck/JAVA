...
// but it needs some attention and does not appear to be 100% correct
  ...
  public AvlNode<T> remove(T x, AvlNode<T> t) {
      ...
      else if(t.left != null) 
      {
         ...
         AvlNode<T> tmp = remove(t.element, t.left);
  ...
  public boolean contains_blk(int Block_num)
  {
      Block_Func block_tmp = new Block_Func(Block_num);
      return contains((T)block_tmp,root);
  }
  ...
  protected T getData(T x, AvlNode<T> t)
  {
       if (t == null)
    {
    ...
    {
      return getData(x, t.left);
    }
    else if (x.compareTo(t.element) > 0)
    {
      return getData(x, t.right); 
    }

    return t.element; // Can only reach here if node was found
  }
  protected T getData(int block_num)
  {
      Block_Func block_tmp = new Block_Func(block_num);
      T block_ref = getData((T)block_tmp, root);
      return block_ref;
  }
  /***********************************************************************/
 ...
 public int get_NodeCount(AvlNode root)
 {
     int result =0 ;
     if(root == null)
         return 0;
     else
         result += get_NodeCount(root.left) + get_NodeCount(root.right);
     return result;
 }
}
